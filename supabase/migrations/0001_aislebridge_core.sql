...
create table public.source_snapshots (
  ...
  unique (tenant_id, id),
  foreign key (tenant_id, source_system_id) references public.source_systems (tenant_id, id) on delete restrict
);
...
create table public.mapping_contracts (
  ...
  foreign key (tenant_id, source_snapshot_id) references public.source_snapshots (tenant_id, id) on delete restrict
);

create table public.change_sets (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null,
  mapping_contract_id uuid not null,
  target_environment text not null check (target_environment = 'SANDBOX'),
  target_version text not null,
  operation_key text not null,
  impact_digest text not null,
  compensation_change_set_id uuid,
  state text not null check (state in ('DRAFT','DRY_RUN_PASSED','APPROVED','EXECUTED','RECONCILED','ROLLED_BACK')),
  unique (tenant_id, id),
  unique (tenant_id, operation_key),
  foreign key (tenant_id, mapping_contract_id) references public.mapping_contracts (tenant_id, id) on delete restrict,
  foreign key (tenant_id, compensation_change_set_id) references public.change_sets (tenant_id, id) on delete restrict
);
...
create table public.evidence_receipts (
  ...
  created_at timestamptz not null default now(),
  unique (tenant_id, id),
  foreign key (tenant_id, change_set_id) references public.change_sets (tenant_id, id) on delete restrict
);
...
alter table public.source_systems force row level security;
alter table public.source_snapshots force row level security;
alter table public.mapping_contracts force row level security;
alter table public.change_sets force row level security;
alter table public.evidence_receipts force row level security;
